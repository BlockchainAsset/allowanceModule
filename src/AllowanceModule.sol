...
contract AllowanceModule {
    ...
    bytes32 public constant ALLOWANCE_MODULE_TYPEHASH = 0x9c2a13446babbd893257bddb8625925562e042a80979eb5328096f0ca00a2a36;
    // keccak256(
    ...
    function executeAllowance(
        ...
        (bytes32 transferHash, bytes memory transferHashData) = generateTransferDataAndHash(_beneficiary, _amount, _expiry, currentNonce);
    ...
    function checkSignature(bytes32 transferHash, bytes memory transferHashData, bytes memory signatures) private view {
    ...
    function transfer(address beneficiary, uint256 amount) private {
        ...
        require(safe.execTransactionFromModule(address(token), 0, data, IEnum.Operation.Call), "Could not execute token transfer");
    }
    ...
    function generateTransferDataAndHash(
        address _beneficiary,
        uint256 _amount,
        uint256 _expiry,
        uint256 _nonce
    ) public view returns (bytes32 dataHash, bytes memory data) {
        uint256 chainId = getChainId();
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_SEPARATOR_TYPEHASH, chainId, this));
        bytes32 transferHash = keccak256(abi.encode(ALLOWANCE_MODULE_TYPEHASH, safe, token, _beneficiary, _amount, _expiry, _nonce));
